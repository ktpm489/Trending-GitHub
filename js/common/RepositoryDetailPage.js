...
import MoreMenuCustom, { MORE_MENU } from './MoreMenuCustom'
const TRENDING_URL = 'https://github.com/'
import { FLAG_TAB } from '../pages/Entry/HomePage'
import BaseComponent from '../base/BaseCommon'
import Share, { ShareSheet, Button } from 'react-native-share';
export default class RepositoryDetailPage extends BaseComponent {
    ...
    renderRightButton(){
        return <TouchableOpacity
            onPress={()=>this.onRightButtonFavoriteClick()}
        >
            <Image
    ...
    renderMoreView() {
        ...
        // custom here
        return <MoreMenuCustom
            ref="moreMenu"
            {...params}
            menus={[MORE_MENU.Save, MORE_MENU.Share]}
            anchorView={this.refs.moreMenuButton}
            onMoreMenuSelect={e => {
                if (e === MORE_MENU.Save) {
                    this.onRightButtonFavoriteClick()
                } else if (e === MORE_MENU.Share) {
                   // alert('Share Menu')
                    this.onOpen()
                }
            }}
    ...
    render(){
       // alert(JSON.stringify(this.props))
        ...
        let shareOptions = {
            ...
            message: this.props.projectModel.item.description,
            url: this.state.url,
            subject: "Share Repository Link" //  for email
        };
                        ...
                        setTimeout(() => {
                            if (typeof shareOptions["url"] !== undefined) {
                                ...
                                if (Platform.OS === "android") {
                                    ...
                                    AlertIOS.alert('Link copied successfully');
                                }
                            }
                        }, 300);
                    }}>Copy Link</Button>
