...
import SearchCell from '../../common/SearchCell'
        ...
        return <View>
            ...
            >
                <View style={{flexDirection:'row',alignItems:'center'}}>
                    ...
                    <Text style={{ fontSize: 18, color: 'white', fontWeight: '400' }}>Search Repository</Text>
                </View>
            </TouchableOpacity>
        </View>
    }
        ...
        let content = this.state.languages.length>0 ?
            ...
            >
                {this.state.languages.map(  (result,i,arr)=>{
                    let item = arr[i];
                    return item.checked? <SearchTabPage key={i} tabLabel={item.name} timeSpan={this.state.timeSpan} {...this.props} />:null;
                })}
...
class SearchTabPage extends Component{
    ...
    componentDidMount = async () =>  {
     await this.loadData(this.props.timeSpan,true);
    ...
    getFetchURL(timeSpan,category){
        // TODO change here
        return 'https://api.github.com/search/repositories?q=tetris+language:actionscript&sort=stars&order=desc'
    ...
    loadData = async (timeSpan,isRefresh) => {
        ...
        try {
           ...
           // console.log(JSON.stringify(responJson))
            if (responJson && responJson.items){
                this.items = responJson.items
                console.log('alert(this.items)', this.items)
                this.getFavoriteKeys();
            } else {
                return 
            }
